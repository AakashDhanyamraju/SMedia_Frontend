...
const Home = () => {
  ...
  useEffect(() => {
    const fetchPosts = async () => {
      try {
        const response = await axios.get('http://localhost:9000/posts/api/postswithusername', {
          headers: {
  ...
  const postLikes = async (id) => {
    try {
      const response = await axios.post(`http://localhost:9000/posts/${id}/like`,{}, {
        headers: {
          'Authorization': `Bearer ${token}`
        }
      });
    ...
    <Fragment>
      ...
      <div className="row">
        ...
        <div className="col-md-10">
          {/* Post Area */}
          <div className="post-area w-75" >
            {/* Render multiple posts */}
            {posts.map((post, index) => (
              <div key={index}>
                <Post content={post.content} media={post.media[0]} userId={post.user} avatar={post.avatar} likes={post.likes.length - 1} user={post.username} postId={post._id} increaselikes={() => postLikes(post._id)} />
                </div>
