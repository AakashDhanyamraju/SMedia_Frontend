...
import { Alert, Button, Card, Col, Container, Modal, Row, Toast } from 'react-bootstrap';
...
const Friends = () => {
  ...
  useEffect(() => {
    ...
    axios.get('http://localhost:9000/friends/friend-requests',{
        headers: {
    ...
    // Fetch sent requests
    axios.get('http://localhost:9000/friends/sent-requests',{
        headers: {
            'Authorization': `Bearer ${token}`
          }
    })
    ...
    // Fetch friends
    axios.get('http://localhost:9000/friends/allfriends',{
        headers: {
            'Authorization': `Bearer ${token}`
          }
    })
  ...
  const handleSendRequest = receiverId => {
    axios.post('http://localhost:9000/friends/send-request', { receiverId }, {
        headers: {
            'Authorization': `Bearer ${token}`
          }
    })
  ...
  const handleAcceptRequest = requestId => {
    axios.put(`http://localhost:9000/friends/accept-request/${requestId}`, {}, {
        headers: {
            'Authorization': `Bearer ${token}`
          }
    })
  ...
  const handleRejectRequest = requestId => {
    axios.put(`http://localhost:9000/friends/reject-request/${requestId}`, {}, {
        headers: {
            'Authorization': `Bearer ${token}`
          }
    })
  ...
  return (
    <div>
                ...
                <Card>
                  ...
                  <Card.Body className='d-flex justify-content-between'>
                    <div className='d-flex text-center'>
                      ...
                      <Card.Title className='mt-2 mx-2'>{request.username}</Card.Title>
                    </div>

                    <div className='d-flex'>
                    <Button className='m-1' onClick={() => handleAcceptRequest(request._id)} variant="primary">Approve</Button>
                    <Button className='m-1' onClick={() => handleRejectRequest(request._id)} variant="danger">Reject</Button>
                    </div>
          ...
          <Col md={4 }  className="mb-4">
                <Card>
                  ...
                  <Card.Body className='d-flex justify-content-between'>
                    <div className='d-flex text-center'>
                      <img className='rounded-circle' height='40px' src='https://st3.depositphotos.com/6672868/13701/v/450/depositphotos_137014128-stock-illustration-user-profile-icon.jpg' />
                      <Card.Title className='mt-2 mx-2'>{friend.username}</Card.Title>
                    </div>
                    {friend.receiverId === currId ? <Button  variant="primary" onClick={() => handleCreateConversation(friend.senderId)}>Message</Button> : <Button  variant="primary" onClick={() => handleCreateConversation(friend.receiverId)}>Message</Button> }
      ...
      <h1 className="text-center mt-4 mb-5">All Users</h1>
        ...
        {/* <h2>All Users</h2> */}
        <Row>
        {allUsers.filter(user => user._id.toString() !== userInfo.id.toString()).map(user => (
              ...
              <Col md={4}  className="mb-4">
                <Card>
                  ...
                  <Card.Body className='d-flex justify-content-between'>
                    <div className='d-flex text-center'>
                      <img className='rounded-circle' height='40px' src='https://st3.depositphotos.com/6672868/13701/v/450/depositphotos_137014128-stock-illustration-user-profile-icon.jpg' />
                      <Card.Title className='mt-2 mx-2'>{user.username}</Card.Title>
                    </div>
                    {/* <Card.Text>
                      {user.mutualFriends} mutual friends
                    </Card.Text> */}
                    <Button onClick={() => handleSendRequest(user._id)} variant="primary">Add Friend</Button>
                  </Card.Body>
                </Card>
              </Col>
